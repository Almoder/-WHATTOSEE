package com.RaProject.whattosee.ui.movies;
...
public class MoviesFragment extends ListFragment {
    ...
    private void setInitialData(){
        ...
        try { maindb = helper.getWritableDatabase();
            ...
            throw mSQLException; }
        Cursor cursor = maindb.rawQuery("select _Key, Image, Title, Year from MainTable where _Key in (select * from Films)", null);
        Cursor genres = maindb.rawQuery("select * from GenresKeys where _Key in (select * from Films)", null);
        cursor.moveToFirst();
        genres.moveToFirst();
