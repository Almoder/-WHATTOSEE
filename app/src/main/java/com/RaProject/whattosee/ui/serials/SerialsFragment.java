package com.RaProject.whattosee.ui.serials;
...
public class SerialsFragment  extends ListFragment {
    ...
    private void setInitialData(){
        ...
        try { maindb = helper.getWritableDatabase();
            ...
            throw mSQLException; }
        Cursor cursor = maindb.rawQuery("select _Key, Image, Title, Year from MainTable where _Key in (select * from Series)", null);
        Cursor genres = maindb.rawQuery("select * from GenresKeys where _Key in (select * from Series)", null);
        cursor.moveToFirst();
        genres.moveToFirst();
